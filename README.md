...
| **Educación parvularia** | Jardines infantiles y salas cuna **sin RBD** (JUNJI, JUNJI VTF, INTEGRA), con el mismo símbolo que los pines; el color de relleno codifica al sostenedor, leído del dato (`slep_sostenedor`): JUNJI e INTEGRA llevan colores propios de la capa, los VTF cuyo sostenedor es un Servicio Local llevan el color de ese Servicio (el mismo que sus pines), con la procedencia de la atribución declarada en la tarjeta y en la exportación (`slep_procedencia`), y los demás un gris de sostenedor distinto de un Servicio Local | 395 | `jq '[.features[]\|select(.properties.tipo_estab >= 5)]\|length' docs/data/parvularia_r5.geojson` |
